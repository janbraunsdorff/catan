...
pub fn verify_buldings(buildings: &Vec<Rc<Building>>, target_coordinates: Vec<(i32, i32)>) -> Result<(), ExecuteError>{
    for (x, y) in target_coordinates {
        let idx = y*100 + x;
        let building = buildings.iter().filter(|x| x.idx == idx).next();
        match building {
            Some(_) => (),
            None => return  Err(ExecuteError{ message: "can not find bulding".to_string(), step: "verify buildings".to_string() }),
        };
    }
