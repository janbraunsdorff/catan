...
use eventque::event::{Event, ExecuteError, EventBox, UndoError};
use eventque as eq;
use game::Game;
...
fn execute<'a>(game_idx: String, new_event: impl Event+Deserialize<'a>+'static) -> Result<(), ExecuteError> {
    // load eventstore
    ...
    events.push(EventBox::from_trait(new_event));
    
    // execute
    let mut game = Game::new();
    for event in events {
        game = match  event.event.execute(game){
            Ok(val) => val,
